...
> 状态: **已完成**（S1～S5）| 依赖: `tech_design` §1 §2 定稿  
...
**任务**:
- [x] 编写 `scrapers/ingest.py`，暴露 `ingest_image(src_path, source_note=None)` 函数，按序执行：
...
- [x] pHash 值存入 DB（需在 `items` 表加 `phash` 字段，或维护内存集合——**建议加字段，方便后续查询**）

> `phash` 已写入 `tech_design.md` §1；S3 实现 ingest 时写入该列。

**验收**:
- [x] 手动放 5 张图（含 1 张与已入库图相似的）至临时目录，批量调用 `ingest_image`
- [x] DB 中出现 4 条记录（重复图被跳过，日志输出 `[SKIP] duplicate: ...`）
- [x] `data/images/thumbnails/` 中缩略图长边 ≤ 400px
...
- [x] 添加基本礼貌：`time.sleep(1~2s)` 请求间隔、`User-Agent` header
- [x] 断点续跑：已在 DB 中的 `source_note` URL 跳过重新下载

**验收**:
- [x] `uv run python scrapers/kym.py --dry-run` 输出解析到的图片数量（应 ≈ 252）
- [x] 正式运行完成后 `SELECT COUNT(*) FROM items` ≥ 200
- [x] 无重复记录（pHash 去重生效）
...
**任务**:
- [x] 手动收集约 250 张图片，存入本地临时目录
- [x] 批量调用：`uv run python scrapers/ingest.py --dir <临时目录>`（需支持目录批量模式）
- [x] 为 `ingest.py` 添加 `--dir` 参数，遍历目录下所有 `.jpg/.png/.webp` 文件

**验收**:
- [x] `SELECT COUNT(*) FROM items WHERE annotation_status = 'raw'` ≥ 500
- [x] `SELECT COUNT(*) FROM items WHERE thumbnail_path IS NOT NULL` = 总记录数（所有图均有缩略图）
- [x] 抽查 10 条记录，`image_path` 与 `thumbnail_path` 文件均实际存在
