...
> 状态: **进行中** | 依赖: Phase 04 ✅  
> 目标: 根治构图搜索精度问题；上线 CLIP 以图搜图（UC-05）；视觉动态排版改版。
...
```
文字搜索：query text → text embed → KNN on item_text_embeddings  (1536-dim)
...
```sql
CREATE VIRTUAL TABLE item_image_embeddings USING vec0(
...
1. 新增 `pipelines/clip_embed.py`，封装 Jina CLIP v2 API 调用
2. 批量对全库 364 张图编码，写入 `item_image_embeddings`
...
- [ ] 全库 `item_image_embeddings` 写入完成，无空值
- [ ] 上传图片可返回 Top-12 视觉结构相似截图
- [ ] UI 加载态、空结果态、错误态均正常展示
- [ ] 文件大小 / 格式校验在前端生效
- [ ] 与文字搜索状态互斥，无状态交叉
